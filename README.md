...
qBitrr includes a WebUI (now powered by React + Vite) that runs inside the main process. It is enabled by default and listens on `http://0.0.0.0:6969/ui` unless you change the host/port in the config.

- **Security**: optionally set `Settings.WebUIToken`; the UI hits unauthenticated `/web/*` endpoints, but API clients should prefer the token-protected `/api/*`.
- **Tabs**:
  - *Processes* – live process list with per-loop restart buttons.
  - *Logs* – view/download log files, with optional auto-tail.
  - *Radarr / Sonarr* – per-instance detail plus aggregated search views.
  - *Config* – edit TOML via forms; saving writes the file and live-reloads qBitrr.
- **Endpoints**: all UI routes proxy the underlying API (`/web/status`, `/web/processes`, `/web/radarr/<category>/movies`, `/web/sonarr/<category>/series`, `/web/config`, etc.).

### Working on the WebUI

The React sources live in `webui/`. The compiled bundle is committed under `qBitrr/static/` and regenerated by `npm run build`, `make syncenv`, and the Docker/CI pipelines. For local UI development:
...
After changes run `npm run build` (or `make syncenv`) so the generated assets in `qBitrr/static/` stay up to date before committing.
