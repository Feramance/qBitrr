 ...
 *
 * The subscriber receives a `release` callback and is expected to invoke it when the
 * network request finishes — wired up in `ArrPosterImage` from the image's
 * `onLoad`/`onError`/abort, plus a watchdog timeout so a stuck request cannot starve
 * the queue forever.
 */

const POSTER_QUEUE_MAX_CONCURRENT = 10;
