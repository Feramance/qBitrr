...
class qBitManager:
    ...
    arr_manager = ArrManager.build_from_config(client)
    ...
    def process_torrents(self) -> None:
        ...
        for key, arr in self.arr_manager.managed_objects.items():
            if key in self.arr_manager.special_categories:
                continue
            arr.api_calls()
        ...
        for torrent in torrents:
            ...
            ) or (torrent.hash in self.arr_manager.managed_objects[torrent.category].timed_skip):
                continue
            elif (
            ...
            # If a torrent was not just added, and the amount left to download is 0 and the torrent is Paused tell the Arr tools to process it.
                ...
                self.arr_manager.managed_objects[torrent.category].import_torrents.append(torrent)
            ...
            # this ensures that we can safelly remove it if the client is reporting the status of the client as "Missing files"
                ...
                self.arr_manager.managed_objects[torrent.category].skip_blacklist.add(torrent.hash)
