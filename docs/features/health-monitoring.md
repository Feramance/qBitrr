...
!!! warning "Not implemented"
    The `[Settings.FFprobe]` section and options such as `CheckVideoCodec`, `AllowedVideoCodecs`, `CheckVideoResolution`, `MinWidth`, `MinHeight`, `CheckVideoBitrate`, `CheckAudioCodec`, `CheckDuration`, `Timeout`, etc. are **not currently implemented** in qBitrr. The only FFprobe-related setting is **`Settings.FFprobeAutoUpdate`**. The examples below are for **future reference** only and have no effect in current versions.
...
#### Audio Validation (not implemented)
...
#### Duration Validation (not implemented)
...
#### Container Format Validation (not implemented)
...
*(The config examples below are not implemented in current qBitrr.)*
...
*(Timeout and the options below are not implemented.)*
...
FFprobe runs in a background thread, so validation does not block other torrents from being processed. *(The config below is not implemented.)*
...
1. Enable auto-download: `Settings.FFprobeAutoUpdate = true`
2. Install manually: `apt-get install ffmpeg` (or place ffprobe in the qBitrr data folder)
3. *(A custom `FFprobePath` is not currently configurable in qBitrr.)*
...
*(A configurable `Timeout` for FFprobe is not currently implemented.)* If validation hangs, ensure the file is not corrupt and consider disabling FFprobe or updating the ffprobe binary.
