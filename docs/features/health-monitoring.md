...
!!! note "Advanced / optional"
    The `[Settings.FFprobe]` section and the options below are **not** generated by `qbitrr --gen-config`. The only FFprobe-related setting in the generated config is `Settings.FFprobeAutoUpdate`. If your codebase reads additional FFprobe options (e.g. `FFprobePath`, `CheckVideoCodec`, `Timeout`), add them manually to `config.toml`. See the [Configuration File Reference](../configuration/config-file.md) for supported keys.
