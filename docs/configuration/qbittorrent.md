...
| `MaxSeedingTime` | `-1` | seconds | Maximum seeding duration before removal action. `-1` disables. Met when qBittorrent `seeding_time` reaches this value, **or** when a stalled upload (`stalledUP`) has `last_activity` older than this limit (`last_activity` of `0` is ignored). HnR still uses actual `seeding_time` and can block deletion. |
