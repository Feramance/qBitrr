...
!!! info "qBittorrent Subcategories (4.6+)"
    For hierarchical qBit categories like `seed/movies`, set `Category` to the **full path** (`Category = "seed/movies"`). To configure a parent once and have qBitrr also manage every child (`seed/movies`, `seed/anime`, ...) enable `MatchSubcategories = true` on the corresponding `[qBit]` / `[qBit-<name>]` section. See [Subcategories](../qbittorrent.md#subcategories-qbittorrent-46).
